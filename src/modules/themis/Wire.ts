...
export class Wire extends THREE.Group {
    ...
    constructor(sourcePos: THREE.Vector3, targetPos: THREE.Vector3, sourceId: string, targetId: string) {
        ...
        // 1. Calculate Bezier points (Quadratic with gravity sag)
        const p1 = sourcePos.clone();
        const p3 = targetPos.clone();

        // Control point: Midpoint + sway/sag
        // Sag depends on horizontal distance?
        const mid = p1.clone().add(p3).multiplyScalar(0.5);
        const dist = p1.distanceTo(p3);
        const sag = Math.max(0.2, dist * 0.3); // Minimum sag or proportional
        mid.y -= sag;
    ...
    public updatePositions(startPos: THREE.Vector3, endPos: THREE.Vector3) {
        ...
        const mid = p1.clone().add(p3).multiplyScalar(0.5);
        const dist = p1.distanceTo(p3);
        const sag = Math.max(0.2, dist * 0.3);
        mid.y -= sag;

        const p2 = mid;

        this.curve.v0.copy(p1);
        this.curve.v1.copy(p2);
        this.curve.v2.copy(p3);
